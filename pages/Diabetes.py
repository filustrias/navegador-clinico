    ...
    with col_hm:
        ...
        fig_hm.update_layout(
            title=dict(text=f"Comorbidades em diabéticos<br>"
                            f"<sup style='color:#AAA'>Total: {n_dm:,} · Diagonal = DM+comorbidade · Off-diagonal = interseção</sup>",
            ...
            for (g1,g2), val in sorted(pares_dm.items(), key=lambda x: -x[1]):
                if val > 0:
                    ...
                    st.markdown(
                        ...
                        f"<span style='font-size:11px;color:#888'>{_p(val,n_dm):.1f}%</span>"
                        f"</div>", unsafe_allow_html=True
                    )
